...
void AP_GPS_UAVCAN::subscribe_msgs(AP_UAVCAN* ap_uavcan)
{
    ...
    auto *fix2_cb = Canard::allocate_arg_callback(ap_uavcan, &handle_fix2_msg_trampoline);
    if (fix2_cb == nullptr) {
        AP_BoardConfig::allocation_error("fix2_cb");
    }
    auto *fix2_sub = new Canard::Subscriber<uavcan_equipment_gnss_Fix2_cxx_iface> {*fix2_cb, ap_uavcan->get_driver_index()};
    if (fix2_sub == nullptr) {
        AP_BoardConfig::allocation_error("fix2_sub");
    }

    auto *aux_cb = Canard::allocate_arg_callback(ap_uavcan, &handle_aux_msg_trampoline);
    if (aux_cb == nullptr) {
        AP_BoardConfig::allocation_error("aux_cb");
    }
    auto *aux_sub = new Canard::Subscriber<uavcan_equipment_gnss_Auxiliary_cxx_iface> {*aux_cb, ap_uavcan->get_driver_index()};
    if (aux_sub == nullptr) {
        AP_BoardConfig::allocation_error("aux_sub");
    }

    auto *heading_cb = Canard::allocate_arg_callback(ap_uavcan, &handle_heading_msg_trampoline);
    if (heading_cb == nullptr) {
        AP_BoardConfig::allocation_error("heading_cb");
    }
    auto *heading_sub = new Canard::Subscriber<ardupilot_gnss_Heading_cxx_iface> {*heading_cb, ap_uavcan->get_driver_index()};
    if (heading_sub == nullptr) {
        AP_BoardConfig::allocation_error("heading_sub");
    }

    auto *status_cb = Canard::allocate_arg_callback(ap_uavcan, &handle_status_msg_trampoline);
    if (status_cb == nullptr) {
        AP_BoardConfig::allocation_error("status_cb");
    }
    auto *status_sub = new Canard::Subscriber<ardupilot_gnss_Status_cxx_iface> {*status_cb, ap_uavcan->get_driver_index()};
    if (status_sub == nullptr) {
        AP_BoardConfig::allocation_error("status_sub");
    }
#if GPS_MOVING_BASELINE
    auto *moving_baseline_cb = Canard::allocate_arg_callback(ap_uavcan, &handle_moving_baseline_msg_trampoline);
    if (moving_baseline_cb == nullptr) {
        AP_BoardConfig::allocation_error("moving_baseline_cb");
    }
    auto *moving_baseline_sub = new Canard::Subscriber<ardupilot_gnss_MovingBaselineData_cxx_iface> {*moving_baseline_cb, ap_uavcan->get_driver_index()};
    if (moving_baseline_sub == nullptr) {
        AP_BoardConfig::allocation_error("moving_baseline_sub");
    }

    auto *relposheading_cb = Canard::allocate_arg_callback(ap_uavcan, &handle_relposheading_msg_trampoline);
    if (relposheading_cb == nullptr) {
        AP_BoardConfig::allocation_error("relposheading_cb");
    }
    auto *relposheading_sub = new Canard::Subscriber<ardupilot_gnss_RelPosHeading_cxx_iface> {*relposheading_cb, ap_uavcan->get_driver_index()};
    if (relposheading_sub == nullptr) {
