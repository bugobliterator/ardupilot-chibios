...
void AP_EFI_DroneCAN::subscribe_msgs(AP_UAVCAN *ap_uavcan)
{
    ...
    auto *status_cb = Canard::allocate_arg_callback(ap_uavcan, &trampoline_status);
    if (status_cb == nullptr) {
        AP_BoardConfig::allocation_error("status_cb");
    }
    auto *status_sub = new Canard::Subscriber<uavcan_equipment_ice_reciprocating_Status_cxx_iface>{*status_cb, ap_uavcan->get_driver_index()};
