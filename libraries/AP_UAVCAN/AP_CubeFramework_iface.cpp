...
#include <cubeframework/handler_list.h>
#include <cubeframework/transfer_object.h>
...
bool CanardInterface::broadcast(const CubeFramework::Transfer &bcast_transfer) {
    if (!initialized) {
        return false;
    }
    WITH_SEMAPHORE(_sem);
...
#if AP_TEST_DRONECAN_DRIVERS
    if (success && (this == &test_iface) && canard_ifaces[0]) {
        // trigger a receive event on the first iface
        canard_ifaces[0]->_event_handle.get_source()->signal(1);
    }
    if (this == &test_iface) {
        test_iface_sem.give();
    }
...
bool CanardInterface::request(uint8_t destination_node_id, const CubeFramework::Transfer &req_transfer) {
    if (!initialized) {
        return false;
    }
    WITH_SEMAPHORE(_sem);
...
bool CanardInterface::respond(uint8_t destination_node_id, const CubeFramework::Transfer &res_transfer) {
    if (!initialized) {
        return false;
    }
    WITH_SEMAPHORE(_sem);
