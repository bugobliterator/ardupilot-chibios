...
#include <cubeframework/interface.h>

class AP_UAVCAN;
class CanardInterface : public CubeFramework::Interface {
...
public:
    ...
    bool broadcast(const CubeFramework::Transfer &bcast_transfer) override;

    /// @brief request message from
    /// @param destination_node_id
    /// @param req_transfer
    /// @return true if request was added to the queue
    bool request(uint8_t destination_node_id, const CubeFramework::Transfer &req_transfer) override;

    /// @brief respond to a request
    /// @param destination_node_id
    /// @param res_transfer
    /// @return true if response was added to the queue
    bool respond(uint8_t destination_node_id, const CubeFramework::Transfer &res_transfer) override;
